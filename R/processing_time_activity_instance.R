...
compute_processing_time <- function(start_dttm, end_dttm, work_schedule, units)  {
  ...
  if(is.null(work_schedule))
    ...
    if(!exists("holidays")){
      warning("Holidays are not accounted for. If you want to add holidays, create proper data object first. (Extra info @Dmitri)")
    } else {
      temp <-
        temp %>% 
        filter(!date %in% holidays$holiday_date)
    }
