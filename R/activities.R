...
activities <- function(eventlog) {
	...
	output <- eventlog %>% group_by(event_classifier) %>% summarize("absolute_frequency" = n_distinct(activity_instance_classifier)) %>% ungroup() %>% arrange(desc(absolute_frequency))
