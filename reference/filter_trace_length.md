...
cases which have a length in this interval.

## Usage

``` r
filter_trace_length(log, interval = NULL, percentage = NULL, reverse = FALSE)
...
- interval, percentage:
  ...
  length 2): A trace length interval. Half open interval can be created
  using [`NA`](https://rdrr.io/r/base/NA.html).  
  `percentage` ([`numeric`](https://rdrr.io/r/base/numeric.html)): A
  percentage p to be used for relative filtering.
...
- `filter_trace_length(log)`: Filters cases for a
  [`log`](https://bupaverse.github.io/bupaR/reference/log.html).

- `filter_trace_length(grouped_log)`: Filters cases for a
  [`grouped_log`](https://bupaverse.github.io/bupaR/reference/grouped_log.html).

## Functions

- `ifilter_trace_length()`: Filter interactively
