...
argument, to which a vector of identifiers can be given. The selection
can be negated with the `reverse` argument.

## Usage

``` r
filter_case(log, cases, reverse = FALSE)
...
- `filter_case(log)`: Filters cases for a
  [`log`](https://bupaverse.github.io/bupaR/reference/log.html).

- `filter_case(grouped_log)`: Filters cases for a
  [`grouped_log`](https://bupaverse.github.io/bupaR/reference/grouped_log.html).

## Functions

- `ifilter_case()`: Filter interactively
